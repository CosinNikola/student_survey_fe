<template>
  <div class="form-page">
    <h1>Izmeni predmet po godini</h1>
    <form>
      ...
      <div class="form-group">
        ...
        <select name="" id="" v-model="this.formData.subject_id" disabled>
...
export default {
  name: "SubjectByYearEdit",
  components: {DashboardButton},
  props: ['id', 'subjectId', 'subjectStudyProgramByYearId'],
  data() {
    return {
      formData: {
        subject_id: this.subjectId,
        id: this.id,
        study_program_by_year_id: this.subjectStudyProgramByYearId,
      },
  ...
  methods: {
    formSubmit(e) {
      ...
      if(this.formData.study_program_by_year_id === "" && this.formData.subject_id === "") {
        ...
        console.log(`sp: ${this.formData.study_program_by_year_id}, subject: ${this.formData.subject_id}`);
        fetch("http://127.0.0.1:8000/api/subject-study-program/" + this.formData.id, {
          method: "PUT",
          ...
          headers: {
            ...
            .then(res => {
              if (res.status === 200) {
                ...
                this.errorMessage = "Greska pri izmeni!";
